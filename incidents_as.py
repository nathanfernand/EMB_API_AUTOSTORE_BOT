...
from main import fetch_incidents_data, get_installation_id
...
def main() -> int:
	...
	df = fetch_incidents_data(installation_id, target_year, log_queue)
	if df.empty:
		print(f"Nenhum dado encontrado para o ano {target_year}.")
		return 0

	output_path = fr"c:\base\API_AUTOSTORE\incidents_{target_year}_diario.xlsx"
	df.to_excel(output_path, index=False, sheet_name="Incidents")
